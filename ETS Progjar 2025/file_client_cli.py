...
class FileClient:
    def __init__(self, server_address='0.0.0.0:7777'):
...
def upload_worker(client, file_size_mb, worker_id):
    local_filename = f"{file_size_mb}MB_file"
    remote_filename = f"testfile_{file_size_mb}mb.dat"
    
    with open(local_filename, 'rb') as f:
        file_data = f.read()
    
    start_time = time.time()
    result = client.remote_upload(remote_filename, file_data)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--server', default='0.0.0.0:7777', help='Server address (host:port)')
